...
export function normalizeEngagementWeights(weights = {}) {
  const normalized = {
    attendance: Number(
      weights.attendance ?? DEFAULT_ENGAGEMENT_WEIGHTS.attendance
    ),
    activity: Number(weights.activity ?? DEFAULT_ENGAGEMENT_WEIGHTS.activity),
    assignment: Number(
      weights.assignment ?? DEFAULT_ENGAGEMENT_WEIGHTS.assignment
    ),
...
export function getEngagementTrend(history = []) {
  ...
  const change =
    normalizeScore(latest.overallScore) - normalizeScore(previous.overallScore);
  let trend = "stable";
