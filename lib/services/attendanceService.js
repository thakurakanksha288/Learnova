...
export class AttendanceService {
  static async recordAttendance(data, decodedToken) {
    const { userId, studentName, email, confidenceScore, normalizedDate, curriculumNodeId } =
      data;
    ...
    const sagaResult = await executeSaga({
      ...
      steps: [
        {
          name: "lock_curriculum",
          execute: async (ctx) => {
            // If no structural curriculum node is bound to this entry, skip locking
            if (!curriculumNodeId) return;

            const curriculumRef = db.collection("curriculum_nodes").doc(curriculumNodeId);
            
            // Run an atomic Firestore transaction to obtain a pessimistic read/write lock on the node
            await db.runTransaction(async (transaction) => {
              const curriculumDoc = await transaction.get(curriculumRef);
              
              if (!curriculumDoc.exists) {
                throw new AppError("Referenced curriculum structure does not exist", 422);
              }

              const curriculumData = curriculumDoc.data();
              // Save structural data snapshot in the saga context for downstream validations
              ctx.curriculumSnapshot = curriculumData;
            });
          },
          compensate: null, // Read-only locking step does not require mutation rollback
        },
        {
