...
export async function syncAttendanceQueue() {
  ...
  try {
    ...
    console.log(
      `[Sync] Attempting to sync ${records.length} queued records...`
    );
    ...
    if (syncedCount > 0) {
      ...
      window.dispatchEvent(
        new CustomEvent("attendance-sync-complete", {
    ...
    console.error(
      "[Sync] Fatal error during attendance sync queue processing:",
      error
    );
  }
}
...
export async function registerBackgroundSync() {
  if (
    ...
    try {
      ...
      console.warn(
        "[Sync] Background sync rejected, falling back to manual flush:",
        error
      );
      syncAttendanceQueue();
    }
